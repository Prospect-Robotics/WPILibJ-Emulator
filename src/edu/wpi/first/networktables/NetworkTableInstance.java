...
public final class NetworkTableInstance implements AutoCloseable {
  ...
  private static NetworkTableInstance s_defaultInstance = new NetworkTableInstance(0);
  ...
  public NetworkTableEntry getEntry(String name) {
    return new NetworkTableEntry(this, 0 /*NetworkTablesJNI.getEntry(m_handle, name)*/);
  }
  ...
  private void startEntryListenerThread() {
    m_entryListenerThread = new Thread(() -> {
      ...
      while (!Thread.interrupted()) {
        ...
        try {
        	Thread.sleep(1000);
          events = new EntryNotification[0];
  ...
  public int addEntryListener(String prefix, Consumer<EntryNotification> listener, int flags) {
    ...
    try {
      if (m_entryListenerPoller == 0) {
        m_entryListenerPoller = 0 /*NetworkTablesJNI.createEntryListenerPoller(m_handle)*/;
        startEntryListenerThread();
      }
      int handle = 0 /*NetworkTablesJNI.addPolledEntryListener(m_entryListenerPoller, prefix, flags)*/;
  ...
  public int addEntryListener(NetworkTableEntry entry,
    ...
    try {
      ...
      int handle = 0 /*NetworkTablesJNI.addPolledEntryListener(m_entryListenerPoller, entry.getHandle(),
          flags)*/;
  ...
  */
  public boolean waitForEntryListenerQueue(double timeout) {
    if (true /*!NetworkTablesJNI.waitForEntryListenerQueue(m_handle, timeout)*/) {
      return false;
    }
    m_entryListenerLock.lock();
    try {
  ...
  private void startConnectionListenerThread() {
    m_connectionListenerThread = new Thread(() -> {
      ...
      while (!Thread.interrupted()) {
        ...
        try {
          events = new ConnectionNotification[0] /*NetworkTablesJNI.pollConnectionListener(this, m_connectionListenerPoller)*/;
  ...
  public int addConnectionListener(Consumer<ConnectionNotification> listener,
    ...
    try {
      ...
      int handle = 0 /*NetworkTablesJNI.addPolledConnectionListener(m_connectionListenerPoller,
          immediateNotify)*/;
  ...
  public boolean waitForConnectionListenerQueue(double timeout) {
    if (true /*!NetworkTablesJNI.waitForConnectionListenerQueue(m_handle, timeout)*/) {
      return false;
    }
    m_connectionListenerLock.lock();
    try {
  ...
  private void startRpcCallThread() {
    m_rpcCallThread = new Thread(() -> {
      ...
      while (!Thread.interrupted()) {
        RpcAnswer[] events;
        try {
          events = new RpcAnswer[0] /*NetworkTablesJNI.pollRpc(this, m_rpcCallPoller)*/;
  ...
  public boolean waitForRpcCallQueue(double timeout) {
    if (true /*!NetworkTablesJNI.waitForRpcCallQueue(m_handle, timeout)*/) {
      return false;
    }
    m_rpcCallLock.lock();
    try {
  ...
  public int getNetworkMode() {
    return 0 /*NetworkTablesJNI.getNetworkMode(m_handle)*/;
  }
  ...
  public ConnectionInfo[] getConnections() {
    return new ConnectionInfo[0] /*NetworkTablesJNI.getConnections(m_handle)*/;
  }
  ...
  public boolean isConnected() {
    return false /*NetworkTablesJNI.isConnected(m_handle)*/;
  }
  ...
  public String[] loadEntries(String filename, String prefix) throws PersistentException {
    return new String[0] /*NetworkTablesJNI.loadEntries(m_handle, filename, prefix)*/;
  }
  ...
  private void startLogThread() {
    m_loggerThread = new Thread(() -> {
      ...
      while (!Thread.interrupted()) {
        LogMessage[] events;
        try {
          events = new LogMessage[0] /*NetworkTablesJNI.pollLogger(this, m_loggerPoller)*/;
        } catch (Exception /*InterruptedException*/ ex) {
          Thread.currentThread().interrupt();
          // don't try to destroy poller, as its handle is likely no longer valid
          wasInterrupted = true;
  ...
  public int addLogger(Consumer<LogMessage> func, int minLevel, int maxLevel) {
    ...
    try {
      ...
      int handle = 0 /*NetworkTablesJNI.addPolledLogger(m_loggerPoller, minLevel, maxLevel)*/;
  ...
  public boolean waitForLoggerQueue(double timeout) {
    if (true /*!NetworkTablesJNI.waitForLoggerQueue(m_handle, timeout)*/) {
      return false;
    }
    m_loggerLock.lock();
    try {
