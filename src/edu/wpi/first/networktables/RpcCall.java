...
public final class RpcCall implements AutoCloseable {
  ...
  public byte[] getResult() {
    byte[] result = new byte[0]; //NetworkTablesJNI.getRpcResult(m_entry.getHandle(), m_call);
    if (result.length != 0) {
      m_call = 0;
    }
    return result;
  ...
  public byte[] getResult(double timeout) {
    byte[] result = new byte[0]; //NetworkTablesJNI.getRpcResult(m_entry.getHandle(), m_call, timeout);
    if (result.length != 0) {
      m_call = 0;
    }
    return result;
